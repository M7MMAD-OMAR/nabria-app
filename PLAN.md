...
- [x] **Palette**: the desktop's accent colour is an optional source, read
      through `org.freedesktop.appearance/accent-color` on the settings portal
      rather than from any one desktop's configuration key -- both backends
      measured implement the portal. Behind the same `follow_desktop_palette`
      switch as the generated palette, not a second one. "Not set" is the
      usual answer and is not a failure.
- [x] Test matrix: Hyprland on real hardware, **KDE Plasma and GNOME from
      stock live images in a virtual machine** -- results in
      `docs/DESKTOPS.md`. Sway is still code paths and containers only. X11 is
      out of scope and the README says so.
...
- [x] Verify the portal on KDE and GNOME. Both advertise
      `org.freedesktop.portal.GlobalShortcuts` at **version 2** --
      `xdg-desktop-portal-gnome` 50.0 and `xdg-desktop-portal-kde` 6.6.4 on
      Fedora 44. Measured rather than read; see `docs/DESKTOPS.md`, which also
      records the one that surprised: the indicator cannot be a layer surface
      on GNOME at all, because Mutter does not implement the protocol, and no
...
- [x] `install.sh --uninstall` removes the block it wrote --
      `shortcut.unbind()`, through `python3 -m nabria.shortcut --unbind` so the
      fence constants have one home. Only between the markers, so a line
      somebody pasted by hand is left alone; includes are searched, because a
