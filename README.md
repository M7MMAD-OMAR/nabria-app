...
[Download the Windows installer](https://github.com/M7MMAD-OMAR/nabria-app/releases/download/v0.6.0/Nabria-0.6.0-windows-x64-setup.exe).
It includes a native Windows desktop and the local transcription engine. Install for your
...
This Windows release is a prerelease and the installer is unsigned.
See [Windows validation and limitations](docs/WINDOWS.md) and the SHA-256
checksum in the [release](https://github.com/M7MMAD-OMAR/nabria-app/releases/tag/v0.6.0).
...
cd nabria-app/packaging && makepkg -si
```
...
The packages come off the way they went on — `dnf remove nabria`, `apt remove
nabria`. The installer has its own:

```sh
scripts/install.sh --uninstall
```

Either way your settings, your models and every transcript you have taken are
**left where they are**. Uninstalling a program is not the same as asking to
lose a 1.6 GB model and everything you ever dictated, and neither can be got
back. To ask for that as well:

```sh
scripts/install.sh --uninstall --purge
```

The setup wizard's "Add it for me" writes into your compositor's
configuration, and `--uninstall` takes that block back out — only the lines
between its own markers, so a shortcut you pasted in yourself is left alone.

You do not have to uninstall anything to delete your data. The settings window
will delete every transcript and the audio kept with them, and will delete a
model to give you the disk back. Both ask twice.
